...
interface IVoilatilePeriphery {
    ...
    function fetchLPPosition(address addr, uint256 positionId) external view returns (
        int256 tickIndex,
        uint128 amount,
        uint128 qTokensEarned
    );

    function fetchSSPosition(address addr, uint256 positionId) external view returns (
        int256 tickIndex,
        uint128 amount,
        uint128 qTokensEarned
    );
